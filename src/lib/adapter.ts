        ...
        nodes.push({
          ...
          parameters: {
            jsCode: [
              ...
              "// التعابير تُبنى بالمُنشئ لا بالحرفيّة: الحرفيّة تحتاج شرطاتٍ مائلة",
              "// تلتهمها سلسلةُ TypeScript قبل أن تصل إلى المحرّك.",
              "const BY_PATH = new RegExp('/d/([-A-Za-z0-9_]{10,})');",
              "const BY_QUERY = new RegExp('[?&]id=([-A-Za-z0-9_]{10,})');",
              "return $input.all()",
              "  .map((i) => {",
              "    const url = Object.values(i.json).find(",
              "      (v) => typeof v === 'string' && v.trim().startsWith('http')",
              "    );",
              "    const s = String(url || '').trim();",
              "    const drive =",
              "      s.includes('drive.google.com') || s.includes('docs.google.com');",
              "    const m = drive ? BY_PATH.exec(s) || BY_QUERY.exec(s) : null;",
              "    return {",
              "      json: { ...i.json, image_url: s, drive_file_id: m ? m[1] : '' },",
              "    };",
              "  })",
